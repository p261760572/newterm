...
int AnalyzeAddiData(glob_msg_stru * pub_data_stru, char *buff)
{
	...
	{
		memcpy(tFieldID, p, 3); p += 3; tFieldID[3] = 0;
		fieldID = HexToDec(tFieldID);
		if(fieldID < 0)break;
		memcpy(tmpBuf, p, lLen); p += lLen; tmpBuf[lLen] = 0;
...
//	memset(outBuff, 0, sizeof(outBuff));
	for(p = buff, pOut = outBuff, flag = 1; p - buff < dLen - lLen - 3;)
	{
		...
		memcpy(pOut, tFieldID, 3); pOut += 3;
		if(0 == memcmp(fieldID, tFieldID, 3))
		{
			if(strlen(fieldBuf) > size - (pOut + lLen - outBuff))
				return -1;
			sprintf(pOut, tmpBuf, strlen(fieldBuf)); pOut += lLen;
			strcpy(pOut, fieldBuf); pOut += strlen(fieldBuf);
			flag = 0;
		}
		else
		{
			sprintf(pOut, tmpBuf, fieldLen); pOut += lLen;
			if(fieldLen > size - (pOut - outBuff))
				return -1;
			memcpy(pOut, p, fieldLen); pOut += fieldLen;
		}
		p += fieldLen;
	}
	*pOut=0x00;
