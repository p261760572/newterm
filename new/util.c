...
int tag(unsigned char *icBuf, int len)
{
     ...
     for(p = icBuf, i = 0; i < len;)
     {
        for(j = 0, ret = -1; TAG[j].fieldLen > 0; j++)
        {
            ...
            {
                p +=  TAG[j].fieldLen; i += TAG[j].fieldLen;
                TAG[j].datalen = *p++; i++;
                memcpy(TAG[j].data, p, TAG[j].datalen);
                p +=  TAG[j].datalen; i += TAG[j].datalen;
				ret = 1;
                break;
            }
        }
	...
	for(p1 = tmpBuf, p = para; *p;)
	{
		if(*p == '#')
		{
			...
			if(*p == '1' || *p == '3') //取数据固定长度，两个字节起起位值，两个字节结束位置(00表示到结尾)
			{
				...
				memcpy(buf, p, 2); p += 2; buf[2] = 0;
				i = atoi(buf);
				memcpy(fieldName, p, i); p += i; fieldName[i] = 0;
				p++;
				...
				if(start < end)
				{
					memcpy(p1, fieldVal + start, end - start); p1 += end - start;
		...
		{
			memcpy(buf, p + 1, 2); p += 3;
			asc_to_bcd((unsigned char *)p1, (unsigned char *)buf, 2, 0); p1++;
		}
		else
		{
			*p1++ = *p++;
	...
	for(p1 = tmpBuf; *p;)
	{
		if(*p == '#')
		{
			...
			if(*p == '1') //取数据固定长度，两个字节起起位值，两个字节结束位置(00表示到结尾)
			{
				...
				memcpy(buf, p + 1, 2); p += 3; buf[2] = 0;
				start = atoi(buf);
				memcpy(buf, p, 2); p += 3; buf[2] = 0;
				end = atoi(buf);
				if(end == 0 || end > fieldLen) end = fieldLen;
				if(start < end)
				{
...
int pin_change(char *para, short fldid, glob_msg_stru *pub_data_stru)
{
	...
	char *p;
//	ICS_DEBUG(0);
	inFldid = fldid; inMode = 0; outMode = 0; inPanFlag = 1; outPanFlag = 1;
	memset(tmpBuf, 0, sizeof(tmpBuf));
	...
	if( pub_data_stru->req_flag)
	{
		...
		memset(outPan, '0', 16); outPan[16] = 0;
		memset(inPan, '0', 16); inPan[16] = 0;
...
//		memset(tmpBuf, 0, sizeof(tmpBuf));
		for(i = 0, p = tmpBuf; i < 2; i++)
		{
			if(TAG[i].datalen > 0)
			{
				bcd_to_asc((unsigned char *)p, (unsigned char *)TAG[i].field, TAG[i].fieldLen * 2, 0); p +=  TAG[i].fieldLen * 2;
				sprintf(p, "%02X", TAG[i].datalen); p += 2;
				bcd_to_asc((unsigned char *)p,(unsigned char *)TAG[i].data, TAG[i].datalen * 2, 0); p += TAG[i].datalen * 2;
			}
		}
		*p=0x00;
